...
class AllowlistMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        user_id, is_private = self._extract_identity(event)
        if user_id is None or not is_private:
    ...
    @staticmethod
    def _extract_identity(event: TelegramObject) -> tuple[int | None, bool]:
        if isinstance(event, Message):
            user_id = event.from_user.id if event.from_user else None
            return user_id, event.chat.type == "private"
        if isinstance(event, CallbackQuery):
            ...
            return event.from_user.id, is_private
        return None, False
